...
import { object, mixed, number, string, ref } from 'yup';
    ...
    <Formik
      ...
      onSubmit={async (values, { setSubmitting }) => {
        if (isLastElement()) {
          setSubmitting(true);
            console.log(values);
            if (values.confirmMethod === 'email') {
                dispatch(authSignup(values.email, values.firstName, values.lastName, values.phoneNumber, values.password));
            }
          setTimeout(() => {
            setSubmitting(false);
          }, 1000);
        ...
        <Form onSubmit={onSubmitHandler}>
          <Steps current={step}>
            {childrenArray.map(child => (
              <Steps.Step key={child.props.label} title={child.props.label} />
            ))}
          </Steps>
